...
ImplementationSingleton(IAPManager);
...
- (void)saveReceipt {
    self.date = [GBAppHelper getCurrentDate];
    
    NSString *fileName = [GBAppHelper stringWithUUID];
...
- (void)sendAppStoreRequestBuyPlist:(NSString *)plistPath {
    ...
    [mineVM loadRequestInAppPurchase:[dic objectForKey:receiptKey]];
    [mineVM setSuccessReturnBlock:^(id returnValue) {
        GBPostNotification(kProductPurchasedSuccessNotification, nil);
        [self removeReceipt];
    }];
}
