...
@interface GBVerificationCodeCell ()
...
@implementation GBVerificationCodeCell

+ (instancetype)cellForTableView:(UITableView *)tableView{
    GBVerificationCodeCell *cell = [tableView dequeueReusableCellWithIdentifier:NSStringFromClass([self class])];
    if (!cell) {
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier{
    if (self=[super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        ...
        if(!_validationCodeView){
            _validationCodeView = [[NNValidationCodeView alloc] initWithFrame:CGRectMake(GBMargin,0, 200, 40) andLabelCount:4 andLabelDistance:8];
            _validationCodeView.defaultColor = [UIColor kSegmentateLineColor];
            _validationCodeView.changedColor = [UIColor kBaseColor];
            [self.contentView addSubview:_validationCodeView];
        }
...
- (void)layoutSubviews {
    ...
    self.validationCodeView.centerY = self.contentView.centerY;
...
- (void)sendCheckNumBtnClicked:(UIButton *)btn {
    if (_delegate && [_delegate respondsToSelector:@selector(GBVerificationCodeCellCheckBtnClick:)]) {
        [_delegate GBVerificationCodeCellCheckBtnClick:self];
    }
