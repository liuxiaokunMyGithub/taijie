    ...
    switch (_message.contentType) {
        ...
        case kJMSGContentTypeImage: {
            ...
            [content thumbImageData:^(NSData *data, NSString *objectId, NSError *error) {
                if(error == nil) {
                    callback(data,self.message.msgId);
        ...
        case kJMSGContentTypeVoice: {
            JMSGVoiceContent *content = (JMSGVoiceContent *)self.message.content;
...
- (UIImage *)resizableBubbleImage {
    ...
    if (_isOutGoing) {
        ...
        bubbleImg = [bubbleImg resizableImageWithCapInsets:UIEdgeInsetsMake(30, 2, 2, 10) resizingMode:UIImageResizingModeTile];
    } else {
        bubbleImg = [UIImage imageNamed:@"inComing_bubble"];
        bubbleImg = [bubbleImg resizableImageWithCapInsets:UIEdgeInsetsMake(30, 10, 2, 2) resizingMode:UIImageResizingModeTile];
    }
    return bubbleImg;
}


- (instancetype)initWithMessage:(JMSGMessage *)message targetAvatar:(UIImage *)targetAvatar isNeedShowTime:(BOOL )isNeedShowTime {
    self = [super init];
    if (self) {
        _msgId = message.msgId;
        IMUserModel *userModel = [[IMUserModel alloc] init];
        [userModel setupWithUser:message targetAvatar:targetAvatar];
        _fromUser = userModel;
        _duration = [message.timestamp floatValue];
        _timeString = [NSString getFriendlyDateString:[message.timestamp doubleValue] forConversation:YES];
        ...
        switch (message.contentType) {
            ...
            case kJMSGContentTypeVideo: {
                ...
                [content videoDataWithProgress:nil completionHandler:^(NSData *data, NSString *objectId, NSError *error) {
                    self.messagemediaPath = content.originMediaLocalPath;
                }];
                _layout = [[MessageLayout alloc] initWithIsOutGoingMessage: _isOutGoing
                                                            isNeedShowTime: isNeedShowTime
                                                         bubbleContentSize: CGSizeMake(120, 160)
