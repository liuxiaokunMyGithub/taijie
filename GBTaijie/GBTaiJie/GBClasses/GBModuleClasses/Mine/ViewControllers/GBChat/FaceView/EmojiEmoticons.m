...
EMOJI_METHOD(faceSavouringDeliciousFood,1F60B);//可口的
EMOJI_METHOD(relievedFace,1F60C);
EMOJI_METHOD(smilingFaceWithHeartShapedEyes,1F60D);//色
EMOJI_METHOD(smilingFaceWithSunglasses,1F60E);// 带墨镜  酷
EMOJI_METHOD(smirkingFace,1F60F);//得意的笑
EMOJI_METHOD(neutralFace,1F610);//中国公民的
EMOJI_METHOD(expressionlessFace,1F611);
EMOJI_METHOD(unamusedFace,1F612);//不高兴的
EMOJI_METHOD(faceWithColdSweat,1F613);//汗
EMOJI_METHOD(pensiveFace,1F614);//沉思的
EMOJI_METHOD(confusedFace,1F615);//困惑
EMOJI_METHOD(confoundedFace,1F616);//惊慌失措
EMOJI_METHOD(kissingFace,1F617);//亲吻
EMOJI_METHOD(faceThrowingAKiss,1F618);//飞吻
...
EMOJI_METHOD(disappointedFace,1F61E);//失望
...
EMOJI_METHOD(anguishedFace,1F627);//痛苦
EMOJI_METHOD(fearfulFace,1F628);//可怕的
EMOJI_METHOD(wearyFace,1F629);//疲惫
EMOJI_METHOD(sleepyFace,1F62A);//睡觉
EMOJI_METHOD(tiredFace,1F62B);
EMOJI_METHOD(grimacingFace,1F62C);
EMOJI_METHOD(loudlyCryingFace,1F62D);//大哭
EMOJI_METHOD(faceWithOpenMouth,1F62E);
EMOJI_METHOD(hushedFace,1F62F);
EMOJI_METHOD(faceWithOpenMouthAndColdSweat,1F630);
EMOJI_METHOD(faceScreamingInFear,1F631);//吃惊
EMOJI_METHOD(astonishedFace,1F632);//晕
EMOJI_METHOD(flushedFace,1F633);//发呆
EMOJI_METHOD(sleepingFace,1F634)//睡觉
EMOJI_METHOD(dizzyFace,1F635);//晕
EMOJI_METHOD(faceWithoutMouth,1F636);//闭嘴
EMOJI_METHOD(faceWithMedicalMask,1F637);
EMOJI_METHOD(grinningCatFaceWithSmilingEyes,1F638);//笑
EMOJI_METHOD(catFaceWithTearsOfJoy,1F639);//哭
