...
#define kXHStoreManagerItemWidth 44
...
- (void)reloadData {
    ...
    for (XHEmotionManager *emotionManager in self.emotionManagers) {
        ...
        sectionButton.titleLabel.font = Fit_Font(14);
        [sectionButton setTitleColor:[UIColor kImportantTitleTextColor] forState:UIControlStateNormal];
        sectionButton.backgroundColor = [UIColor kFunctionBackgroundColor];
...
//    int count = (int)self.emotionManagers.count / (10 * 2);
//    int pageCount = (count + ((self.emotionManagers.count-count )% (10 * 2) ? 1 : 0));
//    [self.sectionBarScrollView setContentSize:CGSizeMake(SCREEN_WIDTH*pageCount, CGRectGetHeight(self.bounds))];
...
- (void)setup {
    ...
    if (self.isShowEmotionStoreButton) {
        ...
        storeManagerItemButton.titleLabel.font = Fit_Font(14);
        ...
        storeManagerItemButton.backgroundColor = [UIColor kBaseColor];
        [self addSubview:storeManagerItemButton];
        [self bringSubviewToFront:storeManagerItemButton];
