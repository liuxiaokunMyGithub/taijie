...
- (instancetype)initWithFrame:(CGRect)frame withTapBlock:(TapBlock)tapBlock
{
    if (self = [super initWithFrame:frame]) {
        ...
        CGFloat btnW = 60;
        CGFloat btnH = 30;
        _countBtn = [[UIButton alloc] initWithFrame:CGRectMake(SCREEN_WIDTH - btnW - 24, SafeAreaTopHeight, btnW, btnH)];
...
//        NSString *filePath = [self getFilePathWithImageName:[GBUserDefaults valueForKey:adImageName]];
...
{
    _filePath = filePath;
    NSString *path = [[NSBundle mainBundle] pathForResource:@"startApp" ofType:@"gif"];
...
- (void)getAdvertisingImage
{
    ...
    NSArray *imageArray = @[adImageName];
...
- (void)downloadAdImageWithUrl:(NSString *)imageUrl imageName:(NSString *)imageName
{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        if ([UIImagePNGRepresentation(image) writeToFile:filePath atomically:YES]) {// 保存成功
            NSLog(@"广告页保存成功");
            [self deleteOldImage];
            [GBUserDefaults setValue:imageName forKey:adImageName];
...
- (void)deleteOldImage
{
    NSString *imageName = [GBUserDefaults valueForKey:adImageName];
    if (imageName) {
        NSString *filePath = [self getFilePathWithImageName:imageName];
        NSFileManager *fileManager = [NSFileManager defaultManager];
