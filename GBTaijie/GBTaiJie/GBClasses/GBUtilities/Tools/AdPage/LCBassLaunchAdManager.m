...
@interface LCBassLaunchAdManager()

@end

@implementation LCBassLaunchAdManager
...
+(LCBassLaunchAdManager *)shareManager{
    static LCBassLaunchAdManager *instance = nil;
    static dispatch_once_t oneToken;
    dispatch_once(&oneToken,^{
        instance = [[LCBassLaunchAdManager alloc] init];
    });
    return instance;
}
...
-(void)example05{
    ...
    //广告图片URLString/或本地图片名(.jpg/.gif请带上后缀)
    imageAdconfiguration.imageNameOrURLString = @"startApp.gif";
    //缓存机制(仅对网络图片有效)
...
-(NSArray<UIView *> *)launchAdSubViews{
    ...
    CGFloat y = iPhoneX ? 54 : [UIScreen mainScreen].bounds.size.height -SCREEN_HEIGHT/3;
...
//    button.titleLabel.font = [UIFont systemFontOfSize:14];
    button.frame = CGRectMake(0,y, SCREEN_WIDTH, SCREEN_HEIGHT/3);
...
-(void)xhLaunchAdShowFinish:(XHLaunchAd *)launchAd{
    ...
    !_launchAdShowFinishBlock ? : _launchAdShowFinishBlock();
}
