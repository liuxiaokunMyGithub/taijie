...
- (instancetype)initWithFrame:(CGRect)frame andLabelCount:(NSInteger)labelCount andLabelDistance:(CGFloat)labelDistance {
    ...
    if (self) {
        ...
        self.changedColor = [UIColor kBaseColor];
        self.defaultColor = [UIColor kSegmentateLineColor];
        [self setupViews];
    }
    return self;
}
...
- (void)setupViews {
    ...
    for (int i = 0; i < self.labelCount; i++) {
        ...
        label.layer.borderColor = [UIColor kSegmentateLineColor].CGColor;
...
- (NNTextField *)codeTextField {
    if (!_codeTextField) {
        ...
        _codeTextField.layer.borderColor = [[UIColor kSegmentateLineColor] CGColor];
