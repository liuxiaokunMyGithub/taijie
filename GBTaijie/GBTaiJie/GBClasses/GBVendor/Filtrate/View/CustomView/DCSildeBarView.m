...
- (DCFiltrateViewController *)filterView {
    if (!_filterView) {
        ...
        _filterView.filtrateItem = [DCFiltrateItem mj_objectArrayWithFilename:@"FiltrateItem.plist"];
        _filterView.view.frame = CGRectMake(SCREEN_WIDTH, 0, SCREEN_WIDTH * 0.65, SCREEN_HEIGHT);
    }
    return _filterView;
}
...
- (void)addSubViews {
    ...
    [UIView animateWithDuration:AnimatorDuration animations:^{
        ...
        weakSelf.filterView.view.x = SCREEN_WIDTH * 0.35;
    }];
    
    //在筛选情况下的确定回调
    self.filterView.sureClickBlock = ^(NSArray *selectArray) {
        GBPostNotification(OrderFiltrateNotification, selectArray);
        [weakSelf tapEvent];
    };
